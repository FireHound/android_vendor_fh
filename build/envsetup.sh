function __print_fh_functions_help() {
cat <<EOF
...
function breakfast()
{
    target=$1
    local variant=$2
    FH_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/fh/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the FireHound model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch fh_$target-$variant
        fi
    fi
    return $?
}
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop ro.fh.device | grep -q "$FH_BUILD"); then
...
function installboot()
{
    ...
    if (adb shell getprop ro.fh.device | grep -q "$FH_BUILD");
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/fh/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $FH_FIXUP_COMMON_OUT ]; then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/fh/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
