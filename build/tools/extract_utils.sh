...
function setup_vendor() {
    ...
    export FIREHOUND_ROOT="$3"
    if [ ! -d "$FIREHOUND_ROOT" ]; then
        echo "\$FIREHOUND_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$FIREHOUND_ROOT/$OUTDIR" ]; then
        mkdir -p "$FIREHOUND_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$FIREHOUND_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$FIREHOUND_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$FIREHOUND_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$FIREHOUND_ROOT"/vendor/fh/build/tools/smali/baksmali.jar
        export SMALIJAR="$FIREHOUND_ROOT"/vendor/fh/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$LINEAGE_ROOT"/vendor/fh/build/tools/"$HOST"/vdexExtractor
    fi
    ...
    if [ ! -f "$FIREHOUND_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$FIREHOUND_TARGET" >/dev/null; then
    ...
    for ARCH in $ARCHES; do
        ...
        elif [[ "$FIREHOUND_TARGET" =~ .jar$ ]]; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$FIREHOUND_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$FIREHOUND_ROOT"/vendor/fh/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$FIREHOUND_ROOT"/"$OUTDIR"/radio
